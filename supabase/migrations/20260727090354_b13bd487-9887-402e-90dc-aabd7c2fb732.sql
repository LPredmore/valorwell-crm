-- Restore the two anonymous Beyond The Yellow / creator-interest website RPCs.
--
-- The original files 20260717225018, 20260718112222 and 20260718124000 were never
-- recorded in supabase_migrations.schema_migrations and were never applied to the
-- shared project. This forward-dated migration re-applies ONLY the parts that are
-- still missing from the live schema, copied verbatim from those files.
--
-- Deliberately NOT re-applied (drift audit 2026-07-27):
--   * every policy on relationship_contacts, relationship_contact_roles,
--     relationship_social_profiles, relationship_influencer_profiles,
--     relationship_organizations and relationship_organization_roles. The live
--     policies are the newer capability-based set
--     (private.crm_has_relationship_permission(...)) installed by later applied
--     migrations. The old files install differently-named permissive policies,
--     which would widen access. Re-applying them would be a regression.
--   * revoke all on table ... from anon. anon currently holds no privileges on
--     any of these tables, so the revokes are no-ops.
--
-- Applied here because the live object is still the older, weaker version:
--   * website_submissions_staff_admin (live: is_staff_or_admin() only, no tenant
--     scoping) -> tenant-aware + subject-reference-scoped version.
--   * "Tenant members can view notes" on crm_notes -> adds the staff-only branch
--     for the new relationship_contact_id column.
...
-- 20260718124000_retire_broad_bty_public_rpc.sql: the legacy broad RPC is not
-- referenced by any live website code path (only by generated types).
