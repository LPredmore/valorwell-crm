...
create or replace function public.list_relationship_campaign_candidates(
  p_filters jsonb default '{}'::jsonb
)
returns jsonb
language sql
...
set search_path = ''
as $$
  select private.list_relationship_campaign_candidates(p_filters);
$$;

revoke all on function private.list_relationship_campaign_candidates(jsonb) from public, anon;
grant execute on function private.list_relationship_campaign_candidates(jsonb) to authenticated, service_role;

revoke all on function public.list_relationship_campaign_candidates(jsonb) from public, anon;
grant execute on function public.list_relationship_campaign_candidates(jsonb) to authenticated, service_role;

comment on function public.list_relationship_campaign_candidates(jsonb) is
  'Returns tenant-scoped donor/funder and BTY referral campaign candidates using one preferred contact per organization and verified referral context when available.';
