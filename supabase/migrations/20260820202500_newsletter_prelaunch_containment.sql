...
-- Compatibility switches remain off throughout PRELAUNCH.
...
-- The malformed five-minute job must not be repaired. Remove it entirely in PRELAUNCH.
do $$
begin
...
-- AI Operations must not treat an intentionally absent PRELAUNCH scheduler as a failure.
...
-- Worker-side privileged newsletter RPCs are service-only.
