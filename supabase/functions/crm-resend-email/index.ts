...
function headerValue(headers: unknown, name: string): string | undefined {
  if (Array.isArray(headers)) {
    const row = headers.find((item) => typeof item === "object" && item !== null &&
...
async function deliver(
  ...
  if (!response.ok || !provider.id) {
    await db.from("crm_email_messages").update({
      ...
      error_message: provider.message ?? "Resend rejected the delivery request",
  ...
  if (input.clientId) {
    await db.from("crm_activity_events").insert({
      ...
      created_by_profile_id: auth.userId,
      metadata: { provider: "resend", email_message_id: queued.id, provider_message_id: provider.id, source: input.source ?? "manual" },
    });
...
async function sendClient(auth: Auth, input: SendInput): Promise<EmailRow> {
  ...
  const messageClass = input.messageClass ?? (input.campaignId ? "ordinary_campaign_follow_up" : "necessary_scheduling");
  await enforcePolicy(auth.tenantId, input.clientId, messageClass, input.source ?? "manual_email", null, input.campaignId);
  return deliver(auth.db, auth, client.email, await settingsFor(auth.db, auth.tenantId), { ...input, messageClass });
}
...
async function processBulk(auth: Auth, bulkSendId: string) {
  ...
  await auth.db.from("crm_bulk_send_logs").update({ status: "in_progress", heartbeat_at: new Date().toISOString() }).eq("id", bulkSendId);

  const recipientType = log.recipient_type || "client";
  const table = recipientType === "staff" ? "crm_bulk_send_staff_recipients" : "crm_bulk_send_recipients";
  ...
  for (const raw of recipients ?? []) {
    ...
    const clientId = recipientType === "client" ? row.client?.id ?? null : null;
    const email = recipientType === "client" ? row.client?.email : row.staff?.profiles?.email;
    try {
      if (!email || !isEmail(email)) throw new Error("No valid email address");
      if (clientId) {
        await enforcePolicy(auth.tenantId, clientId, "ordinary_promotional", "resend_bulk_send", bulkSendId);
      }
      await deliver(auth.db, { tenantId: auth.tenantId, userId: auth.userId }, email, settings, {
        clientId,
        subject: log.subject,
        html: log.body_html,
      ...
      await auth.db.from(table).update({ status: "sent", sent_at: new Date().toISOString(), error_message: null }).eq("id", row.id);
      sent++;
    } catch (sendError) {
    ...
    await auth.db.from("crm_bulk_send_logs").update({ sent_count: sent, failed_count: failed, heartbeat_at: new Date().toISOString() }).eq("id", bulkSendId);
  }
...
async function testConnection(auth: Auth) {
  ...
  if (!isEmail(from) || !isEmail(inbound)) throw new Error("Valid sender and inbound receiving addresses are required");
  const apiKey = Deno.env.get("RESEND_API_KEY") ?? "";
  if (!apiKey) throw new Error("RESEND_API_KEY not configured");
  const response = await fetch(`${RESEND_API}/domains`, { headers: { authorization: `Bearer ${apiKey}`, "user-agent": USER_AGENT } });
  ...
  await auth.db.from("crm_resend_email_settings").update({ connection_status: "connected", last_verified_at: verifiedAt, updated_at: verifiedAt }).eq("tenant_id", auth.tenantId);
  return { connected: true, provider: "resend", fromEmail: from, inboundEmail: inbound, domains, domainStatus: "verified", verifiedAt };
}

async function eventAlreadyExists(db: Db, eventId: string) {
  const { data } = await db.from("crm_email_events").select("id").eq("provider", "resend").eq("provider_event_id", eventId).maybeSingle();
  return !!data;
}

async function handleInbound(db: Db, event: Record<string, unknown>, data: Record<string, unknown>, eventId: string, occurredAt: string, apiKey: string) {
  ...
  const { data: settings } = await db.from("crm_resend_email_settings")
    ...
    .limit(1)
    .maybeSingle();
  if (!settings?.tenant_id) return json({ received: true, ignored: true, reason: "tenant_route_not_found" });
  ...
  if (hintedId) {
    const { data: row } = await db.from("crm_email_messages").select("*").eq("tenant_id", settings.tenant_id).eq("id", hintedId).maybeSingle();
    outbound = row;
  }
  if (!outbound && inReplyTo) {
    const { data: row } = await db.from("crm_email_messages").select("*").eq("tenant_id", settings.tenant_id).or(`provider_message_id.eq.${inReplyTo},provider_thread_id.eq.${inReplyTo}`).limit(1).maybeSingle();
    outbound = row;
  }
  const fromEmail = bareEmail(firstAddress(content.from ?? data.from));
  let clientId = outbound?.client_id ?? null;
  if (!clientId && fromEmail) {
    const { data: client } = await db.from("clients").select("id").eq("tenant_id", settings.tenant_id).ilike("email", fromEmail).limit(1).maybeSingle();
  ...
  if (clientId) {
    ...
    await db.from("clients").update({ last_contact_at: occurredAt, last_contact_channel: "email", last_contact_direction: "received" }).eq("id", clientId);
    const { data: active } = await db.from("crm_campaign_enrollments").select("id").eq("tenant_id", settings.tenant_id).eq("client_id", clientId).eq("status", "active");
    if (active?.length) {
      await db.from("crm_campaign_enrollments").update({ status: "responded", paused_at: occurredAt, pause_reason: "email_response", updated_at: occurredAt }).in("id", active.map((row: any) => row.id));
    }
  }
...
async function handleWebhook(request: Request) {
  ...
  const url = Deno.env.get("SUPABASE_URL") ?? "";
  const service = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "";
  if (!secret || !apiKey || !url || !service) return json({ error: "Resend webhook runtime is not configured" }, 503);
  const rawBody = await request.text();
  const eventId = request.headers.get("svix-id") ?? "";
  const timestamp = request.headers.get("svix-timestamp") ?? "";
  const signature = request.headers.get("svix-signature") ?? "";
  if (!eventId || !timestamp || !signature) return json({ error: "Resend webhook headers are missing" }, 400);
  let event: Record<string, unknown>;
  try {
    event = new Resend(apiKey).webhooks.verify({ payload: rawBody, headers: { id: eventId, timestamp, signature }, webhookSecret: secret }) as unknown as Record<string, unknown>;
  } catch {
    return json({ error: "Invalid webhook signature" }, 401);
  }
  const db = createClient(url, service, { auth: { persistSession: false } });
  if (await eventAlreadyExists(db, eventId)) return json({ received: true, duplicate: true });
  const type = String(event.type ?? "");
  const data = (event.data && typeof event.data === "object" ? event.data : {}) as Record<string, unknown>;
  const occurredAt = String(event.created_at ?? new Date().toISOString());
  if (type === "email.received") return handleInbound(db, event, data, eventId, occurredAt, apiKey);
  const providerId = String(data.email_id ?? data.id ?? "");
  if (!providerId) return json({ received: true, ignored: true, reason: "provider_message_id_missing" });
  const { data: message } = await db.from("crm_email_messages").select("*").eq("provider", "resend").eq("provider_message_id", providerId).maybeSingle();
  if (!message) return json({ received: true, ignored: true, reason: "email_message_not_found" });
  const { error: eventError } = await db.from("crm_email_events").insert({
  ...
  if (map[type]) updates.status = map[type];
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (action === "send") return json({ success: true, message: await sendClient(auth, body as SendInput) });
    if (action === "bulk-send") {
    ...
    if (message.startsWith("SUPPRESSED:")) return json({ error: "Communication suppressed", reason_code: message.slice(11) }, 403);
