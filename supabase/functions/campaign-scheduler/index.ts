...
function providerApplicantEmailContent(applicant: ProviderApplicantData) {
...
function providerApplicantSmsContent(applicant: ProviderApplicantData): string {
  const firstName = applicant.first_name.trim() || "there";
...
async function attachApplicantEmailMessage(
  db: Db,
...
async function processProviderApplicantMessages() {
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const [campaigns, providerApplicants] = await Promise.all([
      processCampaignMessages(),
      processProviderApplicantMessages(),
    ]);
    return json({
      success: true,
      campaigns,
