...
function validateClientJourneyReferences(
  ...
  for (const result of results) {
    ...
    if (suppliedExceptions.size !== suppliedExceptionKeys.length) {
      return { ok: false, error: `Input entity ${entityKey} contains duplicate exception keys.` };
    }

    const relatedExceptions = Array.isArray(result.relatedExceptionKeys) ? result.relatedExceptionKeys : [];
    if (new Set(relatedExceptions).size !== relatedExceptions.length) {
      return { ok: false, error: `Result ${entityKey} contains duplicate relatedExceptionKeys.` };
    }
    for (const key of relatedExceptions) {
      if (!suppliedExceptions.has(key)) return { ok: false, error: `Result ${entityKey} referenced an exception key that was not supplied: ${key}` };
    }
    ...
    if (assessedExceptionKeys.length !== exceptionAssessments.length) {
      return { ok: false, error: `Result ${entityKey} has an exception assessment without an exceptionKey.` };
    }
    if (new Set(assessedExceptionKeys).size !== assessedExceptionKeys.length) {
      return { ok: false, error: `Result ${entityKey} contains duplicate exception assessments.` };
    }
    if (assessedExceptionKeys.length !== suppliedExceptionKeys.length) {
      return { ok: false, error: `Result ${entityKey} did not assess every supplied active exception.` };
    }
    for (const key of assessedExceptionKeys) {
      if (!suppliedExceptions.has(key)) return { ok: false, error: `Result ${entityKey} assessed an exception key that was not supplied: ${key}` };
    }
    for (const key of suppliedExceptionKeys) {
      if (!assessedExceptionKeys.includes(key)) return { ok: false, error: `Result ${entityKey} omitted supplied exception key ${key} from exceptionAssessments.` };
    }
    ...
    if (suppliedFindings.size !== suppliedFindingKeys.length) {
      return { ok: false, error: `Input entity ${entityKey} contains duplicate prior AI finding keys.` };
    }

    const relatedFindings = Array.isArray(result.relatedAiFindingKeys) ? result.relatedAiFindingKeys : [];
    if (new Set(relatedFindings).size !== relatedFindings.length) {
      return { ok: false, error: `Result ${entityKey} contains duplicate relatedAiFindingKeys.` };
    }
    for (const key of relatedFindings) {
      if (!suppliedFindings.has(key)) return { ok: false, error: `Result ${entityKey} referenced a prior AI finding key that was not supplied: ${key}` };
    }
    ...
    if (assessedFindingKeys.length !== priorAssessments.length) {
      return { ok: false, error: `Result ${entityKey} has a prior AI assessment without a findingKey.` };
    }
    if (new Set(assessedFindingKeys).size !== assessedFindingKeys.length) {
      return { ok: false, error: `Result ${entityKey} contains duplicate prior AI finding assessments.` };
    }
    if (assessedFindingKeys.length !== suppliedFindingKeys.length) {
      return { ok: false, error: `Result ${entityKey} did not assess every supplied prior AI finding.` };
    }
    for (const key of assessedFindingKeys) {
      if (!suppliedFindings.has(key)) return { ok: false, error: `Result ${entityKey} assessed a prior AI finding key that was not supplied: ${key}` };
    }
    for (const key of suppliedFindingKeys) {
      if (!assessedFindingKeys.includes(key)) return { ok: false, error: `Result ${entityKey} omitted supplied prior AI finding key ${key}.` };
    }
    ...
    if (sourceDisposition && relatedExceptions.length === 0) {
      return { ok: false, error: `Result ${entityKey} marked an existing source concern without relatedExceptionKeys.` };
    }
    if (disposition === "existing_ai_concern" && relatedFindings.length === 0) {
      return { ok: false, error: `Result ${entityKey} marked an existing AI concern without relatedAiFindingKeys.` };
    }
    if (["none", "new_concern"].includes(disposition) && (relatedExceptions.length > 0 || relatedFindings.length > 0)) {
      return { ok: false, error: `Result ${entityKey} returned existing-concern keys for ${disposition}.` };
    }
    if (sourceDisposition && relatedFindings.length > 0) {
      return { ok: false, error: `Result ${entityKey} mixed a source-exception disposition with prior AI finding links.` };
    }
    if (disposition === "existing_ai_concern" && relatedExceptions.length > 0) {
      return { ok: false, error: `Result ${entityKey} mixed a prior-AI disposition with source exception links.` };
    }
    ...
    if (["none", "stable_existing", "appears_resolved_existing"].includes(disposition) && exceptionAssessments.some((x) => x.assessment === "escalating")) {
    ...
    if (result.noConcern === true && priorAssessments.some((x) => x.assessment === "still_present")) {
      return { ok: false, error: `Result ${entityKey} set noConcern=true while a prior AI finding was assessed still_present.` };
    }
    ...
    if (disposition === "stable_existing" && !relatedExceptionAssessments.some((x) => x.assessment === "stable")) {
      return { ok: false, error: `Result ${entityKey} marked a stable existing concern without a stable related exception assessment.` };
    }
    if (disposition === "escalating_existing" && !relatedExceptionAssessments.some((x) => x.assessment === "escalating")) {
    ...
    if (disposition === "appears_resolved_existing" && !relatedExceptionAssessments.some((x) => x.assessment === "appears_resolved")) {
    ...
    if (disposition === "existing_ai_concern" && !relatedPriorAssessments.some((x) => x.assessment === "still_present")) {
      return { ok: false, error: `Result ${entityKey} marked an existing AI concern without a still_present related prior finding.` };
    }
  }
