...
function corsHeaders(origin: string | null): HeadersInit {
  return {
    "Access-Control-Allow-Origin": isAllowedOrigin(origin) ? origin! : "https://valorwell.org",
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    return json(origin, { ok: false, message: "Please check the form and try again." }, 400);
  }
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    console.error("Clinician interest provisioning is missing Supabase configuration.");
    return json(origin, { ok: false, message: "Something went wrong. Please try again." }, 500);
  }
  ...
  try {
    ...
    const requestIp = forwardedFor || connectingIp || "unknown";
    const requestIpHash = await sha256(`${requestIp}:${Deno.env.get("SB_EXECUTION_ID") ?? "valorwell"}`);

    const { data, error } = await admin.rpc("provision_website_clinician_interest", {
    ...
    if (createdUserId) {
      ...
      if (cleanupError) console.error("Unable to clean up failed clinician auth provisioning.");
    }

    const message = error instanceof Error ? error.message : String(error);
    if (message.includes("rate_limited")) {
      return json(origin, { ok: false, message: "Please try again later." }, 429);
    }
