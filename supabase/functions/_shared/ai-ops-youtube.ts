...
// Read-only: comments are fetched and upserted. Nothing is ever posted back to YouTube.
...
/**
 * Sync recent comments for the configured channel. Videos are read from the
 * channel uploads playlist; comments are upserted by immutable YouTube comment id.
 */
export async function syncYoutubeComments(options: {
  admin: SupabaseClient;
  tenantId: string;
  channelId: string | null;
  btyPlaylistId: string | null;
  maxVideos?: number;
  ...
  const channel = await getJson(
    `${API}/channels?part=contentDetails&id=${encodeURIComponent(options.channelId)}&key=${apiKey}`,
  ...
  const playlist = await getJson(
    `${API}/playlistItems?part=snippet&maxResults=${maxVideos}&playlistId=${encodeURIComponent(uploads)}&key=${apiKey}`,
  ) as { items?: Array<{ snippet?: { resourceId?: { videoId?: string }; title?: string } }> };

  const btyVideoIds = new Set<string>();
  if (options.btyPlaylistId) {
    const bty = await getJson(
      `${API}/playlistItems?part=snippet&maxResults=50&playlistId=${
        encodeURIComponent(options.btyPlaylistId)
      }&key=${apiKey}`,
    ).catch(() => ({})) as { items?: Array<{ snippet?: { resourceId?: { videoId?: string } } }> };
  ...
  for (const item of playlist.items ?? []) {
    ...
    try {
      threads = await getJson(
        `${API}/commentThreads?part=snippet,replies&maxResults=50&order=time&videoId=${videoId}&key=${apiKey}`,
      ) as { items?: CommentThreadItem[] };
    } catch {
      // Comments disabled or unavailable for this video: skip without failing the sync.
      continue;
    }
    ...
    for (const thread of threads.items ?? []) {
      ...
      for (const reply of thread.replies?.comments ?? []) {
        if (reply.id) flat.push({ id: reply.id, snippet: reply.snippet, parentId: top?.id });
      }
      ...
      for (const comment of flat) {
        ...
        const { error } = await options.admin.rpc("ai_ops_upsert_youtube_comment", {
          ...
          p_parent_comment_id: comment.parentId ?? snippet.parentId ?? null,
          p_author_display_name: snippet.authorDisplayName ?? null,
          p_comment_text: snippet.textOriginal ?? snippet.textDisplay ?? "",
          p_published_at: snippet.publishedAt ?? null,
          p_comment_updated_at: snippet.updatedAt ?? snippet.publishedAt ?? null,
  ...
  return { available: true, videosScanned, commentsSeen, commentsUpserted };
}
