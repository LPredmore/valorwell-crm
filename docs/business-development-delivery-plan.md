...
| P25 | COMPLETE (code-only) | Enrollment eligibility | Relationship-only target eligibility evaluates email validity, review, qualification, suppression, duplicate enrollment, response, source permissions, and typed enrollment capability. It performs no enrollment write. |
| P26 | COMPLETE (code-only) | Campaign monitor | Relationship-only monitor summaries cover enrollment, schedule, sends, replies, pauses/stops, failures, suppressions, eligibility changes, and performance. Database-backed monitor queries remain capability-gated. |
| P27 | COMPLETE (code-only) | Execution service source | Separate non-deployed relationship execution contracts and planner cover locking, due work, revalidation, suppression/unsubscribe stops, retries, idempotency, audit boundary, and advancement. No provider, scheduler, database, or clinical import is wired. |
| P28 | COMPLETE (code-only) | Reply processing source and inbox | Relationship-only reply planning covers message matching, enrollment stops, suppression follow-up, interaction/audit context, ownership, next actions, and a capability-gated inbox. No clinical inbox fallback is used. |
| P29 | COMPLETE (code-only) | Suppression service and UI | Relationship-only suppression resolution applies global, email, contact, organization, and campaign precedence with reasons, effective/expiry dates, and audit entries. The workspace remains capability-gated. |
| P30 | COMPLETE (code-only) | Unsubscribe source and public UI | Non-authenticated relationship unsubscribe planning uses opaque token IDs, is idempotent, never alters clinical preferences, and remains inert until the typed unsubscribe capability exists. |

## Reporting, search, and release audit

| Pass | Status | Scope | Completion criteria |
| --- | --- | --- | --- |
| P31 | COMPLETE (code-only) | Reporting | Relationship operational metrics cover organizations, contacts, opportunities, campaigns, enrollment, sends, replies, suppression, unsubscribe, failure, and upcoming-send activity. Pending capabilities render unavailable—not zero—values. |
| P32 | COMPLETE (code-only) | Search | Dedicated relationship search supports name, organization, email, website, initiative, owner, stage, and status fields; results are visibly relationship-scoped and do not alter clinical or inbound search semantics. |
| P33 | IMPLEMENTED — VERIFICATION BLOCKED | Full acceptance audit | Static domain, route, capability, no-database-change, no-deployment, and no-outreach audits are documented. Full test matrix, browser route/accessibility review, and screenshot review remain required in a dependency-capable GitHub-connected environment. |
