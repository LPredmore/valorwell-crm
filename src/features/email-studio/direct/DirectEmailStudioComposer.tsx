...
export type DirectEmailStudioHandle = {
  exportContent: () => Promise<EmailContentDocument | null>;
};

export type DirectEmailStudioComposerProps = {
  initialContent?: EmailContentDocument | null;
  readOnly?: boolean;
  onDirty?: () => void;
};

export const DirectEmailStudioComposer = forwardRef<
  DirectEmailStudioHandle,
  DirectEmailStudioComposerProps
>(function DirectEmailStudioComposer({ initialContent, readOnly = false, onDirty }, ref) {
  ...
  const initialDocument = initialContent?.editorDocument
    || createEmailStudioDocument({ mode: 'direct', scope: 'client', themeKey: initialThemeKey });
  ...
  const [validation, setValidation] = useState<EmailValidationResult>(() =>
    validateEmailStudioEditorDocument(initialDocument, 'direct', 'client'),
  );
  const [snapshot, setSnapshot] = useState<EmailContentDocument | null>(initialContent || null);
  const [previewOpen, setPreviewOpen] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const blocks = useMemo(() => getEmailStudioBlocksForMode('direct'), []);
  ...
  const exportContent = async (openPreview = false): Promise<EmailContentDocument | null> => {
    ...
    if (!current) {
      setError('The Direct Email Studio editor is not ready.');
      return null;
    }
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Direct Email Studio export failed.');
      setStatus('dirty');
      return null;
    }
  };
  ...
  return (
    <div className="space-y-4">
      <EmailStudioToolbar
        ...
        onReset={() => {
          setPreheader('');
          replaceDocument(createEmailStudioDocument({ mode: 'direct', scope: 'client', themeKey }));
        }}
      />
      ...
      <div className="grid gap-4 xl:grid-cols-[220px_minmax(0,1fr)_280px]">
        ...
        <ComposerField
          ...
          onUpdate={() => {
            ...
            setValidation(validateEmailStudioEditorDocument(document, 'direct', 'client'));
            markDirty();
          }}
        />
        <div className="space-y-4">
