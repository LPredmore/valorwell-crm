...
export type MarketingNewsletterEmailStudioHandle = {
  exportContent: () => Promise<EmailContentDocument | null>;
};

export type MarketingNewsletterEmailStudioComposerProps = {
  initialContent?: EmailContentDocument | null;
  readOnly?: boolean;
  onDirty?: () => void;
};

export const MarketingNewsletterEmailStudioComposer = forwardRef<
  MarketingNewsletterEmailStudioHandle,
  MarketingNewsletterEmailStudioComposerProps
>(function MarketingNewsletterEmailStudioComposer({ initialContent, readOnly = false, onDirty }, ref) {
  ...
  const initialDocument = initialContent?.mode === 'newsletter' && initialContent.editorDocument
    ...
    : createEmailStudioDocument({ mode: 'newsletter', scope: 'marketing_newsletter', themeKey: initialThemeKey });
  ...
  const [validation, setValidation] = useState<EmailValidationResult>(() =>
    validateEmailStudioEditorDocument(initialDocument, 'newsletter', 'marketing_newsletter'),
  );
  const [snapshot, setSnapshot] = useState<EmailContentDocument | null>(
    initialContent?.mode === 'newsletter' ? initialContent : null,
  );
  const [previewOpen, setPreviewOpen] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Compliance markup is injected by the delivery renderer so it cannot be removed
  // or accidentally duplicated by authored content.
  const blocks = useMemo(
    () => getEmailStudioBlocksForMode('newsletter').filter((block) => block.kind !== 'compliance-footer'),
    [],
  );
  const variables = useMemo(() => getEmailVariablesForScope('marketing_newsletter'), []);
  ...
  const replaceDocument = (nextDocument: EmailEditorDocument, nextThemeKey = themeKey) => {
    ...
    setSnapshot(null);
    setValidation(validateEmailStudioEditorDocument(nextDocument, 'newsletter', 'marketing_newsletter'));
    setError(null);
    setStatus('loading');
  ...
  const exportContent = async (openPreview = false): Promise<EmailContentDocument | null> => {
    ...
    if (!current) {
      setError('The Marketing Newsletter Email Studio editor is not ready.');
      return null;
    }
    ...
    try {
      ...
      const studioValidation = validateEmailStudioDraft(draft, 'marketing_newsletter');
      ...
      const finalized = await finalizeEmailContentDocument(draft, 'marketing_newsletter');
      ...
      setError(caught instanceof Error ? caught.message : 'Marketing Newsletter Email Studio export failed.');
      setStatus('dirty');
      return null;
    }
  };
  ...
  return (
    <div className="space-y-4">
      <EmailStudioToolbar
        ...
        onReset={() => {
          setPreheader('');
          replaceDocument(createEmailStudioDocument({ mode: 'newsletter', scope: 'marketing_newsletter', themeKey }));
        }}
      />
      ...
      <div className="grid gap-4 xl:grid-cols-[220px_minmax(0,1fr)_280px]">
        ...
        <ComposerField
          ...
          onUpdate={() => {
            ...
            setValidation(validateEmailStudioEditorDocument(document, 'newsletter', 'marketing_newsletter'));
            markDirty();
          }}
        />
        <div className="space-y-4">
