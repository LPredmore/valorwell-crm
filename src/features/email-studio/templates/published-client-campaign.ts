...
import type { PublishedClientCampaignTemplate } from './types';

type RawTemplateRow = CrmEmailTemplateRow & { is_active: boolean };

type ClientCampaignTemplateDatabase = {
  public: {
    Tables: {
      crm_email_templates: {
        Row: RawTemplateRow;
...
const client = supabase as unknown as SupabaseClient<ClientCampaignTemplateDatabase>;

export async function listPublishedClientCampaignTemplates(): Promise<PublishedClientCampaignTemplate[]> {
  ...
  return templates.flatMap((template) => {
    ...
    if (!version || version.content_scope !== 'client' || version.content_mode !== 'campaign') return [];
