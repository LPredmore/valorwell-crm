...
import type { PublishedClientNewsletterTemplate } from './types';

type RawTemplateRow = CrmEmailTemplateRow & { is_active: boolean };

type NewsletterTemplateDatabase = {
  public: {
    Tables: {
      crm_email_templates: {
        Row: RawTemplateRow;
...
const client = supabase as unknown as SupabaseClient<NewsletterTemplateDatabase>;

export async function listPublishedClientNewsletterTemplates(): Promise<PublishedClientNewsletterTemplate[]> {
  ...
  return templates.flatMap((template) => {
    ...
    if (!version || version.content_scope !== 'client' || version.content_mode !== 'newsletter') return [];
