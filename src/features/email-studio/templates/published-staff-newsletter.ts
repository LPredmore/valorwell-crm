...
import type { PublishedStaffNewsletterTemplate } from './types';

type RawTemplateRow = CrmEmailTemplateRow & { is_active: boolean };

type StaffNewsletterTemplateDatabase = {
  public: {
    Tables: {
      crm_email_templates: {
        Row: RawTemplateRow;
...
const client = supabase as unknown as SupabaseClient<StaffNewsletterTemplateDatabase>;

export async function listPublishedStaffNewsletterTemplates(): Promise<PublishedStaffNewsletterTemplate[]> {
  const { data: templateData, error: templateError } = await client
    .from('crm_email_templates')
    .select('*')
    .eq('content_scope', 'staff')
    .eq('content_mode', 'newsletter')
    .eq('status', 'published')
    .eq('is_active', true)
  ...
  return templates.flatMap((template) => {
    ...
    if (!version || version.content_scope !== 'staff' || version.content_mode !== 'newsletter') return [];
