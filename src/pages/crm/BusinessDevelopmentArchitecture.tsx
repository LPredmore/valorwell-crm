    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-muted-foreground">Application architecture, database readiness, final implementation acceptance, and controlled activation status for the non-clinical relationship domain.</p>
        </div>
      ...
      <Card>
        <CardHeader><CardTitle>Final acceptance status</CardTitle><CardDescription>Implementation acceptance and outbound activation are independent release decisions.</CardDescription></CardHeader>
        <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {[
            ['Architecture established', true, 'The relationship domain and clinical separation are defined and enforced.'],
            ['Application implemented', true, 'Passes 1–13 are merged with tenant-scoped workflows, safety, delivery boundaries, search, and reporting.'],
            ['Database contracts available', allCapabilitiesAvailable, capabilities.isLoading ? 'Checking the capability snapshot.' : `${availableCount} of ${capabilityTotal} capability contracts are available.`],
            ['Implementation accepted', true, 'Pass 14 completed the release audit, security review, regression checks, worker review, and final acceptance record.'],
            ['Outbound activation', false, 'Intentionally held. Provider readiness, campaign execution, and enrollment delivery must be enabled through a separate controlled activation decision.'],
          ].map(([label, ready, detail]) => <div className="rounded-lg border p-3" key={label as string}><p className="text-sm font-medium">{label}</p><Badge className="mt-2" variant={ready ? 'default' : 'outline'}>{ready ? 'Accepted' : 'Held'}</Badge><p className="mt-2 text-xs text-muted-foreground">{detail}</p></div>)}
        </CardContent>
      </Card>

      {capabilities.isError && <div role="alert" className="rounded-md border border-destructive/40 bg-destructive/5 p-4 text-sm">Capability status could not be loaded. Database-dependent functions remain disabled until status can be verified.</div>}
