...
function medianDaysBetween(rows: Row[], fromCol: string, toCol: string): number {
  const spans: number[] = [];
  for (const r of rows) {
    const a = r[fromCol]; const b = r[toCol];
    if (!a || !b) continue;
...
export const supabaseReportsRepository: ReportsRepository = {
  async journeyFunnel() {
    ...
    const stageRows: Record<LifecycleStage, Row[]> = Object.fromEntries(
      LIFECYCLE_STAGES.map((s) => [s, [] as Row[]]),
    ) as Record<LifecycleStage, Row[]>;
    for (const c of clients) {
      if (!c.lifecycle_stage) continue;
      try {
    ...
    return LIFECYCLE_STAGES.map((stage) => ({
      ...
      medianDays: medianDaysBetween(stageRows[stage], 'created_at', 'lifecycle_stage_changed_at'),
    }));
  },

  async atRiskMetrics() {
    const clients = await fetchAll('clients',
      'at_risk, at_risk_since, lifecycle_stage');
    ...
    const meta = await fetchAll('crm_client_canonical_meta',
      'client_id, risk_reason, at_risk_marked_at');
    const reasonCounts = new Map<string, number>();
    const atRiskIds = new Set(atRisk.map((c) => c.id ?? c.client_id));
    for (const m of meta) {
      if (!m.risk_reason) continue;
      if (m.client_id && !atRiskIds.has(m.client_id)) continue;
  ...
  async closureMetrics() {
    const clients = await fetchAll('clients', 'closure_reason');
    const map = new Map<string, number>();
    for (const c of clients) {
      if (!c.closure_reason) continue;
      let label = c.closure_reason as string;
  ...
  async campaignPerformance() {
    const [campaigns, enrollments, logs] = await Promise.all([
      fetchAll('crm_campaigns', 'id, name'),
      fetchAll('crm_campaign_enrollments', 'campaign_id, status'),
      fetchAll('crm_campaign_step_logs', 'step_id, status, enrollment_id'),
    ]);
    // Map step -> campaign via a step lookup.
    const steps = await fetchAll('crm_campaign_steps', 'id, campaign_id');
  ...
  async taskPerformance() {
    const tasks = await fetchAll('crm_tasks',
    ...
    for (const t of tasks) {
      ...
      const isOverdue = t.due_at && new Date(t.due_at).getTime() < now && t.status !== 'completed';
  ...
  async exceptionMetrics() {
    const rows = await fetchAll('crm_exceptions',
